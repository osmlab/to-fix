...
echo "
    COPY (select ST_AsText(wkb_geometry) from unconnected_major1) to stdout DELIMITER ',' HEADER CSV;
" | psql -U postgres osmi > osmi-tasks/unconnected_major1.csv

echo "
    COPY (select ST_AsText(wkb_geometry) from unconnected_major2) to stdout DELIMITER ',' HEADER CSV;
" | psql -U postgres osmi > osmi-tasks/unconnected_major2.csv

echo "
    COPY (select ST_AsText(wkb_geometry) from unconnected_major5) to stdout DELIMITER ',' HEADER CSV;
" | psql -U postgres osmi > osmi-tasks/unconnected_major5.csv

echo "
    COPY (select ST_AsText(wkb_geometry) from unconnected_minor1) to stdout DELIMITER ',' HEADER CSV;
" | psql -U postgres osmi > osmi-tasks/unconnected_minor1.csv

echo "
    COPY (select ST_AsText(wkb_geometry) from unconnected_minor2) to stdout DELIMITER ',' HEADER CSV;
