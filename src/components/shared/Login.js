...
import { userLogin, getUserDetails } from '../../actions';
...
class Login extends Component {
  ...
  renderLoginState() {
    const { authenticated, username, avatar } = this.props;
    ...
    if (authenticated) {
      return (
        <div className='pad1x col12 truncate clearfix mobile-cols'>
          ...
          <button onClick={() => console.log('openSettings')} className='col6 icon sprocket button quiet small animate'>Settings</button>
        </div>
      );
    } else {
      return (
